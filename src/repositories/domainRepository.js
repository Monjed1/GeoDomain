...
function cleanMetadata(candidate, requestContext) {
  return {
    ...
    estimatedMonthlySearchVolume: candidate.searchDemand?.estimatedMonthlySearchVolume ?? '',
    estimatedCpcUsd: candidate.searchDemand?.estimatedCpcUsd ?? '',
    demandScore: candidate.searchDemand?.demandScore ?? '',
    marketSegment: candidate.searchDemand?.marketSegment || '',
    estimatedBuyerPool: candidate.buyerPool?.estimatedBusinesses ?? '',
    buyerPoolScore: candidate.buyerPool?.buyerPoolScore ?? '',
    cityTier: candidate.buyerPool?.cityTier || '',
    estimatedLeadValueUsd: candidate.leadValue?.estimatedLeadValueUsd ?? '',
    leadValueScore: candidate.leadValue?.leadValueScore ?? '',
    leadCloseDifficulty: candidate.leadValue?.closeDifficulty || '',
    brandabilityScore: candidate.brandability?.brandabilityScore ?? '',
    pronounceableScore: candidate.brandability?.pronounceableScore ?? '',
    exactMatchIntentScore: candidate.brandability?.exactMatchIntentScore ?? '',
    liquidityScore: candidate.liquidity?.liquidityScore ?? '',
    sellSpeed: candidate.liquidity?.sellSpeed || '',
    trademarkRiskLevel: candidate.trademarkRisk?.level || '',
    trademarkRiskFlags: JSON.stringify(candidate.trademarkRisk?.flags || []),
    scoreReasons: JSON.stringify(candidate.reasons || []),
